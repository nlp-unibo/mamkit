...
from mamkit.data.datasets import MMUSEDFallacyContext, MMUSEDFallacy, InputMode
...
if __name__ == '__main__':
    ...
    save_path = Path(__file__).parent.parent.resolve().joinpath('results', 'mmused-fallacy', 'afc', 'text_only_roberta')
    if not save_path.exists():
        save_path.mkdir(parents=True)

    base_data_path = Path(__file__).parent.parent.resolve().joinpath('data')

    loader = MMUSEDFallacyContext(task_name='afc',
    ...
    df = loader.data
    text_data = loader._get_text_data(df)
    print(text_data[0][0])

    audio_data = loader._get_audio_data(df)
    print(audio_data[0][0])

    text_audio = loader._get_text_audio_data(df)
    print(text_audio[0][0])
