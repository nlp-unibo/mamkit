...
from mamkit.components.collators import PairMultimodalCollator, PairTextCollator, PairAudioCollator
from mamkit.components.data import MArg, InputMode
from mamkit.components.data import PairVocabBuilder, PairMultimodalProcessor, PairAudioTransformerExtractor
...
if __name__ == '__main__':
    ...
    save_path = Path(__file__).parent.parent.resolve().joinpath('results', 'marg', 'arc', 'text_audio_lstm_hubert')
    ...
    for seed in config.seeds:
        ...
        for split_info in loader.get_splits(key='mancini-et-al-2022'):
            processor = PairMultimodalProcessor(text_processor=PairVocabBuilder(tokenizer=config.tokenizer,
                                                                                embedding_model=config.embedding_model,
                                                                                embedding_dim=config.text_embedding_dim),
                                                audio_processor=PairAudioTransformerExtractor(
            ...
            model = PairBiLSTM(vocab_size=len(processor.text_processor.vocab),
