...
from mamkit.components.collators import MultimodalCollator, TextTransformerCollator, AudioCollator
from mamkit.components.data import UKDebates, InputMode
from mamkit.components.data import MultimodalProcessor, AudioTransformerExtractor
...
if __name__ == '__main__':
    ...
    save_path = Path(__file__).parent.parent.resolve().joinpath('results', 'ukdebates', 'text_audio_roberta_hubert')
    ...
    config = MMTransformerConfig.from_config(key=ConfigKey(dataset='ukdebates',
                                                           input_mode=InputMode.TEXT_AUDIO,
                                                           task_name='asd',
                                                           tags={'anonymous', 'roberta', 'hubert'}))
    ...
    for seed in config.seeds:
        ...
        for split_info in loader.get_splits(key='mancini-et-al-2022'):
            processor = MultimodalProcessor(audio_processor=AudioTransformerExtractor(
